...
import (
	...
	"github.com/tetronoz/proglog/internal/auth"
...
func testSetup(t *testing.T, fn func(*Config)) (rootClient api.LogClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	t.Helper()
...
func testUnauthorized(t *testing.T, _, client api.LogClient, config *Config) {
	...
	if produce != nil {
		t.Fatalf("produce respsonse should be nil")
	}
	...
	if consume != nil {
		t.Fatalf("consume respsonse should be nil")
	}
	gotCode, wantCode = status.Code(err), codes.PermissionDenied
	if gotCode != wantCode {
		t.Fatalf("got code: %d, want: %d", gotCode, wantCode)
